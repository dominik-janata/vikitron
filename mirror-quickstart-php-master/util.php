...
function bootstrap_new_user() {
  ...
  //$timeline_item->setText("Pick the REPLY item and say the word you want to see definition");
  $timeline_item->setHtml('<article>
   <section>
     <div class="text-x-large" style="">
       <p class="yellow">VIKITRON<sub>.com</sub></p>
       <center>Input your<br>query ...</center>
     </div>
   </section>
 </article>');
  
  $replyMenuItem = new Google_MenuItem();
  
  $replyMenuItem->setAction("REPLY");
  
  $timeline_item->setMenuItems(Array(
      $replyMenuItem
  ));
  ...
  subscribe_to_notifications($mirror_service, "timeline",
    $_SESSION['userid'], $base_notification_url . "/notify.php");
}
